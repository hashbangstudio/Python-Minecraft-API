...
class TestEvent(unittest.TestCase):
    ...
    def testRepresentation(self):
        data = [0,14,15,16,1,1]
        evnt = BlockEvent(data[0],data[1],data[2],data[3],data[4],data[5])
        expectedString = "BlockEvent(%s, %d, %d, %d, %d, %d)"%(data[0],data[1],data[2],data[3],data[4],data[5])
        rep = repr(evnt)
        self.assertTrue(rep, expectedString)
